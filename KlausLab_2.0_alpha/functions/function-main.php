...
function normal_style_script() { 
    ...
    // 插件样式
    wp_enqueue_style( 'support', get_template_directory_uri() . '/css/support.css', array(), '1.0', false ); 
    // vue.min.js   
    ...
    // 工具库
    wp_enqueue_script( 'myUtil', get_template_directory_uri() . '/js/util.js', array(), '1.0', false ); 
	// 先将ajaxurl变数设定好
...
function footer_script(){		
    ...
    // 支持
    wp_enqueue_script( 'support', get_template_directory_uri() . '/js/support.js', array(), '1.0', false );
    // 配置
...
function styles_scripts() {
    if( ! is_admin() ) { // 前台加载的脚本与样式表
		...
		if( is_mobile() == true ){
			wp_enqueue_script( 'flexible', get_template_directory_uri() . '/js/flexible.js', array(), 'lastet', false );
		}
    } else { // 后台加载的脚本与样式表
        // 取消加载 jquery 脚本
...
if ( !is_admin() ) {
    ...
    function qiniu_cdn_replace($html){
        ...
        $cdn_exts   = 'png|jpg|jpeg|gif|ico|webp|js|css'; //扩展名（使用|分隔）
