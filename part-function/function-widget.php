...
{
  global $wpdb;
  $countsOfLogin = $wpdb->get_results("SELECT e.user_nicename, e.display_name, e.user_url, e.user_email, d.user_id, d.meta_value FROM $wpdb->users AS e, $wpdb->usermeta AS d WHERE  e.ID = d.user_id AND d.meta_key = 'last_login' AND to_days(now()) - to_days(meta_value) <= 30 LIMIT $friends_num");
