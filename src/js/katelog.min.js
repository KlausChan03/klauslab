(function (root, factory) {
    ...
    let defaultOpts = {
      linkClass: 'kl-catelog-link',
      linkActiveClass: 'kl-catelog-linkl-active',
      supplyTop: 0,
      ...
      if (tree) {
        ...
        for (let i = 0; i < tree.length; i++) {
          if (isEqual(tree[i].parent, _parent)) {
            hasChild = true;
            ul += `<li><div class="${option.linkClass} kl-catelog-level-${tree[i].level}" data-target="${tree[i].id}">` + tree[i].name + '</div>';
    ...
    let styleText = `
      .kl-catelog-list > ul { position: relative; }    
    `
